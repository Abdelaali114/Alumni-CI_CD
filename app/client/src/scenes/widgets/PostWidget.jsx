...
const PostWidget = ({
 ...
 useEffect(() => {
  const fetchCurrentReaction = async () => {
    const response = await fetch(`http://app-server-1:3001/posts/${postId}/reaction?userId=${loggedInUserId}`, {
      method: "GET",
      headers: {
 ...
 const patchLike = async (reaction) => {
  const response = await fetch(`http://app-server-1:3001/posts/${postId}/like`, {
    method: "PATCH",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
...
const patchUnlike = async () => {
  const response = await fetch(`http://app-server-1:3001/posts/${postId}/unlike`, {
    method: "PATCH",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
    ...
    <WidgetWrapper m="2rem 0">
      ...
      {picturePath && (
        <img
          ...
          src={`http://app-server-1:3001/assets/${picturePath}`}
        />
      )}
      ...
      <SharePopup
        ...
        url={`http://app-server-1:5173/posts/${postId}`} // Adjust the URL as needed
      />
    </WidgetWrapper>
  );
};
